      ...
      <view class="order-info-cnt" v-for="(product,index) in products" :key="index">
        ...
        <view>
          ...
          <view class="order-info-cnt-inf" v-if="product.jshd_pre_rate!==''">
            <view class="order-info-cnt-info">预定金比例：<span style="color: #ED2856; padding-right: 5px;">{{(product.jshd_pre_rate*1).toFixed(2)}}%</span> | 尾款：<span style="color: #ED2856;">{{parseFloat(product.jshd_pre_amount).toFixed(2)}}%</span>  </view>
          </view>
        </view>
      </view>
